...
sudo sed -i "s|placeholder_user_music_dir|$USER_MUSIC_DIR|g" "$INSTALL_DIR/$MPD_NOTIFIER_SH"
sudo sed -i "s|placeholder_mpd_host|192.168.1.80|g" "$INSTALL_DIR/$MPD_NOTIFIER_SH"
sudo sed -i "s|placeholder_user_cache_dir|$USER_CACHE_DIR|g" "$INSTALL_DIR/$MPD_NOTIFIER_SH"
sudo sed -i "s|placeholder_image_dir|$USER_MUSIC_DIR|g" "$INSTALL_DIR/$MPD_WEBSERVER_PY"
sudo sed -i "s|placeholder_path_to_mpd-webserver-py|$INSTALL_DIR/$MPD_WEBSERVER_PY|g" "/etc/systemd/system/$IMAGE_SERVER_SERVICE"
sudo sed -i 's|placeholder_working_directory|$INSTALL_DIR|g' /etc/systemd/system/image-server.service

# Replace placeholders in mpd-notifier.sh with full home directory paths
